...
from .models import CidadesRegiao, Cidades, Regioes

class CidadeRegiaoForm(forms.ModelForm):    
    ...
    def clean(self):
        ...
        if regiao and cidade:
            # Consulta para verificar duplicidade
            query = CidadesRegiao.objects.filter(
                regiao=regiao,
                cidade=cidade
            )
